...
public class AttackTurret : Turret
{
    ...
    private void Awake()
    {
        ...
        typeObject = GameObjects.AttackTurret;
    }
    ...
    {   
        ...
        if(hitCollidersTargets.Length > 0)
            foreach (var item in hitCollidersTargets)
            {
                ...
                SpawnPoint spawnPoint = SpawnPoints.Instance.SpawnPointsList.Find(x => x.transform == item.transform);
                if(character != null)
                {
                ...
                {
                    targets.RemoveAll(x => x == null);
                    targets.RemoveAll(x => x == spawnPoint.transform);
                    targets.Add(spawnPoint.transform);
